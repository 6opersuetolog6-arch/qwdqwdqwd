...
async def sync_command(ctx):
    ...
    try:
        ...
        await ctx.send(f"в хуй? - {len(synced)}")
...
class ApplicationModal(discord.ui.Modal, title="Заявка в Mercuri Famq"):
    def __init__(self, target_channel: discord.TextChannel):
        ...
        self.nick = discord.ui.TextInput(
            label="Nick Name | Static ID",
            placeholder="Пример Ivan | 666",
            required=True,
            max_length=32
        )
        self.static_id = discord.ui.TextInput(
            label="Возраст",
            placeholder="Пример: 14 лет",
            required=True,
            max_length=10
        )
        self.age = discord.ui.TextInput(
            label="Откуда узнали о семье",
            placeholder="Пример: маркет",
            required=True,
            max_length=3
        )
        ...
        self.details = discord.ui.TextInput(
            label="В какой фракции находитесь",
            placeholder="Пример: LSPD",
            required=True,
            max_length=200,
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="👤 Nick Static", value=self.nick.value, inline=True)
        embed.add_field(name="🆔 Возраст", value=self.static_id.value, inline=True)
        embed.add_field(name="🎂 Откуда узнали о семье", value=self.age.value, inline=True)
        embed.add_field(name="📛 Ваше имя в IRL", value=self.real_name.value, inline=True)
...
class ApplicationControlView(discord.ui.View):
    ...
    async def approve_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        welcome_message = (
            "🎉 **Поздравляем!** Вы приняты в **Mercuri Famq**!\n\n"
            "📌 **ВАЖНО СДЕЛАТЬ СЛЕДУЮЩЕЕ:**\n"
            "1️⃣ Напишите в ЛС боту команду **`/профиль`** и заполните свой Nick Name и Static ID.\n"
...
async def mass_move(interaction: discord.Interaction):
    ...
    spam_message = "СРОЧНЫЙ СБОР! ВЫ ПЕРЕМЕЩЕНЫ В ОТДЕЛЬНЫЙ ВОЙС"
